...
def plot_decode_timewise(
    ...
    for label, (work, traffic, color, marker) in methods.items():
        ...
        if label == "FlashAttention kernel":
            axs.text(x_line[1] * 1.25, y_line[1] * 0.9, 'MAX', color='black', fontsize=10)
        elif label == "Matmul kernel":
            axs.text(x_line[1] * 1, y_line[1] * 0.6, 'MAX', color='black', fontsize=10)
        elif label == "Xformers kernel":
            axs.text(x_line[1] * 0.4, y_line[1] * 1.1, 'MAX', color='black', fontsize=10)
    ...
    # Label axes
    axs.set_ylabel('Performance (Flop/s)', fontsize=12)
    axs.set_xlabel('Arithmetic Intensity (FLOP/byte)', fontsize=12)
    axs.text(0.15, peak_work_single_precision * 0.0008, 'Memory-bound', color='black', fontsize=10, fontweight='bold')
    axs.text(intersection_x * 1.15, peak_work_single_precision * 0.0008, 'Compute-bound', color='black', fontsize=10, fontweight='bold')
