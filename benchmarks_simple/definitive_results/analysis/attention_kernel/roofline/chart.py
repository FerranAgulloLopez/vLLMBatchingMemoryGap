...
def plot_decode_timewise(
    ...
    # Create figure
    l = 300
    fig, axs = plt.subplots(figsize=(5.5, 3.5), constrained_layout=True)
    ...
    for label, (work, traffic, color, marker) in methods.items():
        ...
        if label == "FlashAttention kernel":
            ...
            axs.text(x_line[1] * 0.9, y_line[1] * 0.5, 'MAX', color='black', fontsize=12)
        elif label == "Xformers kernel":
            axs.text(x_line[1] * 0.33, y_line[1] * 1.35, 'MAX', color='black', fontsize=12)

    # Set log scales
    axs.set_xscale('log')
    axs.set_yscale('log')
    axs.xaxis.set_major_locator(ticker.LogLocator(base=10.0, subs=[1.0], numticks=12))
    axs.yaxis.set_major_locator(ticker.LogLocator(base=10.0, subs=[1.0], numticks=12))
    ...
    # Label axes
    axs.set_ylabel('Performance (Flop/s)', fontsize=14)
    axs.set_xlabel('Arithmetic Intensity (FLOP/byte)', fontsize=14)
    axs.text(0.2, peak_work_single_precision * 0.0006, 'Memory \n bound', color='black', fontsize=12, fontweight='bold')
    axs.text(intersection_x * 1.6, peak_work_single_precision * 0.0006, 'Compute \n bound', color='black', fontsize=12, fontweight='bold')
    ...
    fig.legend(handles=legend_handles, loc='upper center', ncol=2, frameon=False, fontsize=14, bbox_to_anchor=(0.57, 1.3))
