...
CUDA_GRAPH_RUNNER: Dict[str, Set] = {
    ...
    'kv_split': {
        'void flash_fwd_splitkv_kernel<Flash_fwd_kernel_traits<128, 64, 128, 4, false, false, cutlass::half_t, Flash_kernel_traits<128, 64, 128, 4, cutlass::half_t> >, false, false, false, false, true, false, false, false>(Flash_fwd_params)',
    }
}
...
def extract_experiment_metric(path: str) -> Dict[str, float]:
    ...
    output['cuda_graph_runner_kv_split_cuda_time'] = 0
    for item in cuda_graph_runner['children']:
        item_name: str = item['entry']['name']
        cuda_time_us: float = item['entry']['cuda_time_us']
        if item_name in CUDA_GRAPH_RUNNER['matrix_multiplications']:
            output['cuda_graph_runner_matrix_multiplications_cuda_time'] += cuda_time_us
        elif item_name in CUDA_GRAPH_RUNNER['mem_transfers']:
            output['cuda_graph_runner_mem_transfers_cuda_time'] += cuda_time_us
        elif item_name in CUDA_GRAPH_RUNNER['kv_split']:
            output['cuda_graph_runner_kv_split_cuda_time'] += cuda_time_us
...
def plot_results(
    ...
    results_kv_split = __prepare_lines(
        results,
        'batch_size',
        'cuda_graph_runner_kv_split_cuda_time',
        'output_len'
    )
    ...
    for label, y_line in {
        ...
        'KV split time': results_kv_split[0][2],
