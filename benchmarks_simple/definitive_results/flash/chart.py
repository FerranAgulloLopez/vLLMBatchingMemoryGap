...
CUDA_GRAPH_RUNNER: Dict[str, Set] = {
    ...
    'flash_attention': {
        'void flash_fwd_splitkv_kernel',
        'void flash_fwd_splitkv_combine_kernel',
    },
    'vllm': {
        'void vllm::rotary_embedding_kernel',
        'void vllm::reshape_and_cache_flash_kernel',
...
def extract_experiment_metric(path: str) -> Dict[str, float]:
    ...
    output['cuda_graph_runner_flash_attention_cuda_time'] = 0
    ...
    for item in cuda_graph_runner['children']:
        ...
        if breaked:
            continue
        for selector in CUDA_GRAPH_RUNNER['flash_attention']:
            if selector in item_name:
                output['cuda_graph_runner_flash_attention_cuda_time'] += cuda_time_us
                breaked = True
                break
        if breaked:
            continue
...
def plot_results(
    ...
    plt.savefig(os.path.join(path, f'total_cuda_time_{title}'), bbox_inches='tight')
    ...
    plt.savefig(os.path.join(path, f'total_cuda_time_speedup_{title}'), bbox_inches='tight')
    ...
    results_flash_attention = __prepare_lines(
        results,
        'batch_size',
        'cuda_graph_runner_flash_attention_cuda_time',
        'output_len'
    )
    ...
    for label, y_line in {
        ...
        'Flash attention time': results_flash_attention[0][2],
    ...
    plt.savefig(os.path.join(path, f'cuda_time_types_{title}'), bbox_inches='tight')



def main():
    results: List[Dict[str, float]] = extract_results('llama-2-7b')
    plot_results(results, '.', 'llama_2_7b')
